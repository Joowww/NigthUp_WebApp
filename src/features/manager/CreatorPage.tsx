...
export function CreatorPage({ }: CreatorPageProps) {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-screen">Cargando panel...</div>;
  }
  ...
  return (
    <div className="min-h-screen px-6 py-8 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-lg text-muted-foreground/80">
              Gestiona tus eventos, analiza métricas y haz crecer tu comunidad.
            </p>
          </div>
          <Button
            onClick={() => {
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-colors">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Asistentes Totales</p>
        ...
        {/* Lista de eventos */}
        <div>
          <h2 className="mb-4 text-foreground">Tus Eventos</h2>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {events.map((event) => (
              <Card
                key={event.id || event._id}
                className={`group overflow-hidden border-border bg-card transition-all duration-300 cursor-pointer
                  hover:scale-[1.02] hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(121,40,202,0.3)]
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground mb-4">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>{event.attendees || event.participants?.length || 0} personas</span>
                      </div>
                    </div>
                  </div>
          ...
          {events.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <h3 className="mb-2 text-foreground">No tienes eventos aún</h3>
              <p className="text-muted-foreground mb-4">
                Crea tu primer evento y empieza a gestionar tu audiencia
              </p>
              <Button
                onClick={() => {
                  setEditingEvent(null);
