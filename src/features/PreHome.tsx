...
const OnboardingFlow = () => {
  ...
  const [filteredComunidades, setFilteredComunidades] = useState(cities);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);

  // Categorías con un poco más de estilo visual si quisieras añadir iconos luego
  const categorias = ['Trap', 'Reagge', 'Edgy', 'Tecno', 'Reaggeton', 'House', 'Loofy', 'Funk', 'Pop', 'Indie', 'Rock', 'Metal', 'Trendy', 'Pop con ñ', 'España 2000'];

  useEffect(() => {
    if (search) {
      setFilteredComunidades(
        cities.filter(comunidad =>
          comunidad.toLowerCase().includes(search.toLowerCase())
        )
      );
    } else {
      setFilteredComunidades(cities);
    }
  }, [search]);
          ...
          <div className="h-64 overflow-y-auto space-y-2 mb-6 pr-2 custom-scrollbar">
            {filteredComunidades.map((c) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-200 border ${comunidad === c
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          {categorias.map((categoria) => (
            <button
              ...
              className={`p-4 border rounded-xl transition-all duration-200 font-medium ${intereses.includes(categoria)
